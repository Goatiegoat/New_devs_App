...
interface RevenueData {
    property_id: string;
    total_revenue: number | string;  // API returns string to preserve Decimal precision
    currency: string;
...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', debugTenant, showRaw }) => {
    ...
    // Convert to number for display if it's a string, preserving decimal precision
    const revenueAsNumber = typeof data.total_revenue === 'string' 
        ? parseFloat(data.total_revenue) 
        : data.total_revenue;
    
    // Parse string directly to preserve precision without float arithmetic
    const displayTotal = typeof data.total_revenue === 'string'
        ? parseFloat(data.total_revenue)
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300">
            ...
            <div className="p-6">
                    ...
                    <div>
                        ...
                        <div className="flex items-baseline gap-2 mt-1">
                            <span className="text-3xl font-bold text-gray-900 tracking-tight">
                                {data.currency} {typeof data.total_revenue === 'string'
                ...
                <div className="mt-4 h-6">
                    {typeof data.total_revenue === 'string' && showRaw && (
